...
pid = fork do
  wp.close
  STDIN.reopen(rp)
  exec("yad", "--notification", "--listen", '--image=user-available', '--text="@Now"', '--item-separator=`', "--command=kill -HUP #{PPID}")
end
...
begin
  File.open("/var/run/atnow-#{Process:UID.eid}.pid", "w") {|f| f.print PPID}
  Process.waitpid(pid)
ensure
  File.delete("/var/run/atnow-#{Process:UID.eid}.pid")
end